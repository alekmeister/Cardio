...
class Workout {
  date = new Date();
  id = (Date.now() + '').slice(-10); // создаем уникальный id, обычно берут из библиотек
  clickNumber = 0;
...
class App {
  ...
  _toggleClimbField() {
    inputClimb.closest('.form__row').classList.toggle('form__row--hidden'); // closest - ближайший родитель, toogle - переключение
  ...
  _displayWorkoutOnSidebar(workout) {
    ...
    <li class="workout workout--${workout.type}" data-id="${workout.id}">
      ...
      <div class="workout__details">
        ...
        <input class="workout__value" value=${workout.duration}></span>
  ...
  _addWorkoutsToLocalStorage() {
    localStorage.setItem('workouts', JSON.stringify(this.#workouts)); // добавление в локальное хранилище. Они не имеют прототипной цени т.к. конвертируем в строку.
  }
